...
AccountList Follower::followed(){
    return AccountList("SELECT %s FROM users, favorites "
        ...
        "AND favorites.user_id = " + number(user->id) + " "
...
std::vector<std::string> Follower::followers(){
    DB::Result r = DB::query(
        ...
        "AND favorites.ref = " + number(user->id));
...
bool Follower::following(int uid){
    return favHelper("artist", uid, user->id);
}

bool Follower::favorited(int tid){
    return favHelper("track", tid, user->id);
}

TrackList Follower::favorites(){
    return TrackList(
        "SELECT %s FROM users, favorites WHERE favorites.type = 'track' "
        "AND favorites.ref = tracks.id AND favorites.user_id = " + number(user->id) + " "
        "ORDER BY users.name ASC");
}
