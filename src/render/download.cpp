...
void Http::trackArt(int tid, Art::Size sz){
    ...
    Track t(tid);
    if(!t) return Html::notFound("Track");
    Art::Format f = sz == Art::Full ? art.getFormat() : Art::PNG;
    ...
    Http::download((string)"/downloads/art/"+(sz==Art::Medium?"medium/":sz==Art::Thumbnail?"thumb/":"")+number(tid)+(sz==Art::Full?"":".png"), t.artist().name()+" - "+t.title()+ext, mime);
}
